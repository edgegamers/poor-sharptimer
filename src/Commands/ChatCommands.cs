        ...
        public async Task RankCommandHandler(CCSPlayerController? player, string steamId, int playerSlot, string playerName, bool sendRankToHUD = false, int style = 0)
        {
            try
            {
                ...
                Server.NextFrame(() =>
                {
                    ...
                    // If rank changed, refresh scoreboard placement cache and update Actain tag (if available).
                    cachedPlacements.Remove(playerSlot);
                    if (Mixin.Actain != null && player != null && player.IsValid)
                    {
                        Mixin.Actain.getTagService().SetTag(player, ranking, false);
                        Mixin.Actain.getTagService().SetTagColor(player, GetRankColorForChat(player)[0], false);
                    }
